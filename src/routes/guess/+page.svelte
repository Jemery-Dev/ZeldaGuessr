<script>
    ...
    onMount(function () {
        ...
        if (gameList.length > 0) {
            selectRandomLocation();
            startTime = new Date(); // Start the timer when the game starts
            gameStarted = true;
        }
    });
</script>
    ...
    <div class="content-center items-center justify-center">
        ...
        {#if currentLocation}
            ...
            {:else if isCorrect === false}
                <p>Incorrect! The correct answer was {currentLocation.name}.</p>
            {/if}

            <p>Score: {gameScore} || {selectedType.charAt(0).toUpperCase() + selectedType.slice(1)}
                remaining: {gameList.length}</p>
            <p>Total attempts: {totalAttempts}</p>
        {/if}
    </div>
{/if}
